function routes(config) {
  ...
  function target(req) {
    //This function is for DRY
    return controllers.login.targetOBJ(
      req.query.email,
      req.query.firstName,
      req.query.lastName,
      req.query.target
  ...
  app.get("/", async function(req, res) {
    res.locals.layout = "blank";
    if (req.session.user && typeof req.query.displayNormal === "undefined") {
      //Does the session exist? Recall that the express-session is using its own id system in the background to hook into a specific user profile.
  ...
  app.get("/iframe", async function(req, res) {
    ...
    if (req.session.user && typeof req.query.displayNormal === "undefined") {
      ...
      let package = await controllers.editor.readPage(req.route.path);
      res.render("pages/iframe", package);
    }
  }); //end '/'
  ...
  app.post("/login", async function(req, res) {
    response = await controllers.login.logOn(
      req.body.username,
      req.body.password,
      target(req)
    );
    if (response.attempt) {
      await login(req);
      res.redirect(response.string);
    } else {
      res.render(response.template, response.parameters);
    }
  }); // end app.post
  ...
  app.post("/register", async function(req, res) {
    response = await controllers.login.register(
      req.body.username,
      req.body.password,
      req.body.email,
      target(req)
